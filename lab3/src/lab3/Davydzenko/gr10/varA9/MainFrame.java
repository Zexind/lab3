...
public class MainFrame extends JFrame
{
		...
		{ 
			...
			{ 
				...
				JOptionPane.showMessageDialog(null,
						"Выполнил студент 10 группы Давыденко Евгений Дмитриевич", "информация", JOptionPane.INFORMATION_MESSAGE);
				
			}
			};

		infoMenuItem = infoMenu.add(infoAction); 
		...
		infoMenuItem.setEnabled(true); 			
			JLabel labelForFrom = new JLabel("X изменяется на интервале от:"); 
					...
					{ public void actionPerformed(ActionEvent ev) {
						...
						saveToTextMenuItem.setEnabled(false); saveToGraphicsMenuItem.setEnabled(false); 
						searchValueMenuItem.setEnabled(false); 
						getContentPane().validate(); } });
	...
	protected void saveToTextFile(File selectedFile) { try {
		...
		out.print("Многочлен: "); for (int i=0; i<coefficients.length; i++) 
		{ out.print(coefficients[i] + "*X^" + (coefficients.length-i-1)); 
		if (i!=coefficients.length-1) out.print(" + "); } 
		out.println(""); out.println("Интервал от " + data.getFrom() + " до " 
	...
	try {
		...
		System.out.println(
				"Ошибка преобразования строки '" + args[i] + "' в число типа Double");
		System.exit(-2); }
